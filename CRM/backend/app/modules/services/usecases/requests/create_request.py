...
class CreateServiceRequestUseCase(BaseCommandUseCase):
    """
    Create a service request.

    Business Rules:
    - Service must exist and be active
    - Notifies admins about new request
    - Auto-generates request_number (REQ-000001)
    """
    ...
    def execute(self, user_id: str, service_id: int, description: str = None,
        ...
        Returns:
            UseCaseResult with created request
        """
        ...
        if not service.is_active:
            return UseCaseResult.fail('Service is not available', 'SERVICE_INACTIVE')
        ...
        return UseCaseResult.ok({'request': request.to_dict()})

    def _notify_admins(self, request: ServiceRequest):
        """Send notification to admins about new request (same company only)"""
        try:
